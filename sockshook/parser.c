...
static int make_netent(char *value, struct networkent **ent);
	...
	if (nowords > 0) {
		...
		if (!strcmp(words[0], "path")) {
			...
			if ((nowords != 3) || (strcmp(words[1], "="))) {
				show_msg(MSGERR, "Malformed configuration pair on line %d in configuration "
					   "file, \"%s\"\n", lineno, savedline);
...
static int handle_defuser(struct parsedfile *config, int lineno, char *value) {
	...
	if (currentcontext->defuser != NULL) {
		if (currentcontext == &(config->defaultserver)) 
			show_msg(MSGERR, "Default username may only be specified once for default server, at line %d "
				   "in configuration file\n", lineno);
		else 
...
/* Construct a networkent given a string like                             */
/* "198.126.0.1[:portno[-portno]]/255.255.255.0"                      */
int make_netent(char *value, struct networkent **ent) {
	char *ip;
	char *subnet;
   char *startport = NULL;
   char *endport = NULL;
   ...
   if ((*ent = (struct networkent *) malloc(sizeof(struct networkent)))
      == NULL) {
...
{
	struct serverent *ent,*next_ent;
	struct networkent *net,*next_net;	
...
int list_servers(struct parsedfile *config, char ***argv) 
{
	*argv = NULL;
	...
	char **argvv = NULL;
	if (total_count) {
		argvv = (char**)malloc((total_count + 1) * sizeof(char*));

		ent = config->paths;
		while (ent != NULL) {
			sprintf(buff, "%s:%d", ent->address, ent->port);
			argvv[index] = strdup(buff);
			show_msg(MSGDEBUG, "list: %s\n", argvv[index]);
			ent = ent->next;
			index++;
		}
	}
	
	show_msg(MSGDEBUG, "set argv result(%d)\n", total_count);
	*argv = argvv;
	show_msg(MSGDEBUG, "set argv result finished.\n");
...
int pick_server(struct parsedfile *config, struct serverent **ent, 
		struct in_addr *ip, unsigned int port, int startindex) {
	struct networkent *net;	
	char ipbuf[64];
